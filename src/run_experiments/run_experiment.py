...
def real(folder_name):
    ...
    if config_m["MIMO"]:
        for ch in range(config_m["num_channels"]):
            g_ch = hf.create_group("{}".format(ch))
            g_ch.create_dataset(
                "z_test_hat",
                data=z_test_hat[ch],
                compression="gzip",
...
def real_series(folder_name):
    for p in range(4):
        ...
        # build model knowing noiseSTD
        crsae_series.build_model(config_d["noiseSTD"])
        ################################################
        # get lambda
        print("lambda before training:", crsae.get_lambda())
        print("noiseSTD before training:", crsae.get_noiseSTD())
        ...
        if config_m["MIMO"]:
            for ch in range(config_m["num_channels"]):
                ...
                g_ch.create_dataset(
                    "z_series_hat",
                    data=z_series_hat[ch],
                    compression="gzip",
                    compression_opts=9,
                )
                g_ch.create_dataset(
                    "y_series_hat",
                    data=y_series_hat[ch],
                    compression="gzip",
                    compression_opts=9,
                )
                g_ch.create_dataset(
...
def lcsc_simulated(folder_name):
    ...
    lcsc.trainer.set_num_train_reset(config_m["num_train_reset"])
    ...
    if lcsc.trainer.get_close():
        We_true = np.load("{}/experiments/{}/data/H_true.npy".format(PATH, folder_name))
